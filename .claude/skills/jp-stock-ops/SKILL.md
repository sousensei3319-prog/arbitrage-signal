...
**触ってよいのは `jp_*` / `dashboard/` / `data/jp_stocks/` / `jp-stock*.yml` / `pages.yml` のみ**。
...
- **このサンドボックス(Claude Code)からは Yahoo Finance にも github.io にも到達できない
  (プロキシ403)**。実データの疎通確認は必ず GitHub Actions ランナー上の実行結果
  (ジョブログ・コミット・run の conclusion)で行うこと。WebFetch/curlでの直接確認は不可。
...
- **JPX空売り比率・信用残の需給レイヤー追加**: 無料で取得可能な範囲でのファンダ/需給補強。
  現状のスクリーナーは売買代金(価格×出来高)のみで、需給の裏付け(誰が買っているか)は無い
...
- **算出可能な事実のみ**を書く(集中度・z値・売買代金・値動き%・時間帯など、CSVから直接
  計算できる数値に基づく記述のみ)
- **ニュース理由・需給の断定は書かない**。データに裏付けが無い「なぜ上がったか」は
  「※要確認」等と明示し、断定的な表現(「〜が理由」「〜の思惑」等)を使わない
- 末尾に**投資助言ではない旨**を必ず付す(既存の `_commentary()` 末尾のパターンを踏襲)
- 材料(ニュース・信用残・空売り比率等)を実装で保持していない限り、それに触れる記述は
  「本データに含まれず未検証」と明記する
