...
| シグナル | ロジック | 実装 |
|---|---|---|
| **コンセンサス新規参入** | 上位N人のうちK人以上が同一銘柄・同方向に新規ポジション。30日検証: +24h平均+1.4%/勝率60% | `smart_money_tracker.py` (チャート付き通知) |
| **VIP単独ムーブ** | 実現PnL上位8人は1人でも$50k+の新規/転換/クローズで即通知 | `smart_money_tracker.py` (`vip_addresses.csv`) |
| **注目急上昇スクリーン** | 人間系の週間売買代金の前週比で「ローテーション初動」を検知 | `smart_money_report.py` (週次配信) |
| **拒否権フィルター** | 合算ネットポジション$2M+に逆らう既存シグナル候補を除外 | `smart_money/sm_filter.py` (統合済み) |
...
### 2026-07の重要な発見: 勝ち組人間の戦場は「株式perp」へ移行中

500人分析で、人間系スマートマネーの主戦場が `xyz:` 系マーケット
(HL上のビルダー展開 米株/指数/商品perp — TSLA, NVDA, SP500, GOLD等) に
移りつつあることが判明した (直近の注目急上昇上位がほぼ株式perp)。
週次レポートの「xyz:シェア」でこの移行を定点観測している。
若い戦場ほど先行者が有利 — 元ツイートの「戦場選び」はクリプトの外側も含む。
