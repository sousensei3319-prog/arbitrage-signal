"""
米国株 1分足コレクター (us_stock_fetch.py)

Yahoo Finance の非公式チャートAPI (v8/finance/chart) を標準ライブラリのみで叩き、
指定銘柄の1分足を data/us_stocks/ 以下にCSVで蓄積する。jp_stock_fetch.py の忠実な
移植 (タイムゾーンをJST→ETに変更、.T自動補完を削除しただけで、429適応バックオフ・
重複排除・query1/query2フォールバックのロジックは同一)。
...
背景・設計:
  ...
  - NYSE/Nasdaqの休場日・時間外に実行しても新規バーが単に無いだけ
...
設定 (環境変数、pushイベントで空文字になるケースに備えて `or` で既定値にフォールバック):
  US_TICKERS           Yahoo Finance形式のカンマ区切り (既定: AAPL,NVDA,MSFT)
  RANGE                取得レンジ (既定 5d)
  INTERVAL             足種 (既定 1m)
  DATA_DIR             CSV出力先 (既定 data/us_stocks)
  FETCH_DEADLINE_MIN   全体デッドライン分・ハング防止 (既定 5)
  SLEEP_SEC            銘柄間の基本スリープ秒・連続リクエストブロック回避 (既定 0.4)

429対策 (JP版と同じ適応的バックオフ):
  - 429(Too Many Requests)を検知したら、以降の銘柄で段階的にスリープを延長する。
    延長は実行全体を通じて維持し、失敗が続くほど伸ばす (上限あり)。1銘柄の429ごとに
    リトライはしない — 次回実行の重複排除つき再取得で自然に埋まる設計を踏襲する。

実行:
  python us_stock_fetch.py
  US_TICKERS="AAPL,NVDA" python us_stock_fetch.py
"""
...
# Config (環境変数で調整可)
# ============================================================
DEFAULT_TICKERS = "AAPL,NVDA,MSFT"  # サンプル
...
def append_bars(path, ticker, timestamps, quote, known):
    ...
    for i, ts in enumerate(timestamps):
        ...
        new_rows.append((et, ts, ticker, o, h, l, c, v))
        known.add(ts)
    ...
    with open(path, "a", newline="", encoding="utf-8") as f:
        w = csv.writer(f)
        if is_new:
            w.writerow(["timestamp_et", "epoch", "ticker",
...
def load_tickers():
    """取得対象を解決: US_TICKERS(env) > UNIVERSE_FILE(code列) > DEFAULT_TICKERS。"""
    env = os.environ.get("US_TICKERS")
    if env:
...
def main():
    ...
    print(f"完了: 新規{total_new}本を{DATA_DIR}に保存 (失敗{fail}銘柄, うち429={n_429}件, "
