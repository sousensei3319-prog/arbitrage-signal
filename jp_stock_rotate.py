"""
日本株 1分足 ローリング窓+月次アーカイブ化 (jp_stock_rotate.py)

850銘柄化でリポジトリが肥大化するのを防ぐため、ライブファイル {code}_T_1m.csv を
直近 ROLLING_DAYS 営業日分だけに切り詰め、溢れた古い分は月次gzipアーカイブ
{code}_T_1m_YYYYMM.csv.gz へ退避する (標準ライブラリのgzipモジュールのみ使用)。

設計:
  - 「営業日」はカレンダー日数ではなく、CSVに実際に存在する日付(timestamp_jstの日付部分)の
    種類数で数える。休場日はそもそもバーが無いので自然に除外される。
  - アーカイブは月(YYYYMM)ごとに1ファイル。同じ月に複数回ローテーションしても
    gzipの追記(多重メンバー方式。Pythonのgzipモジュールは読込時に多重メンバーを
    透過的に連結して展開するため問題ない)でヘッダーは新規ファイル作成時のみ書く。
  - アーカイブはダッシュボード/スクリーナーからは読み込まない設計 (後検証用のみ)。
    このスクリプトはライブCSVの整合性(スクリーナーが読む対象)を変えない
    ―― 切り詰め後も列構成・ソート順は既存と同じ。
  - 1分足(*_T_1m.csv)のみが対象。日足/週足/月足は1日1本ペースで増加が緩やかなため
    (2年分でも数百行程度)、ローテーション対象外 (PMの設計方針どおり)。

設定 (環境変数、pushイベントでinputsが空文字になるケースに備えて `or` で既定値):
  DATA_DIR       対象ディレクトリ (既定 data/jp_stocks)
  ROLLING_DAYS   ライブファイルに残す営業日数 (既定 7)
  ARCHIVE_SUFFIX ライブファイルの拡張子直前の識別子 (既定 _T_1m, universeのINTERVAL=1m前提)

実行: python jp_stock_rotate.py
"""
...
LIVE_SUFFIX = "_T_1m.csv"  # jp_stock_fetch.py の出力ファイル名規則 (code + _ + INTERVAL + .csv)
...
def rotate_one(path):
    ...
    dates = sorted({r["timestamp_jst"][:10] for r in rows if r.get("timestamp_jst")})
    if len(dates) <= ROLLING_DAYS:
        return 0, len(dates)  # ローテーション不要 (窓内に収まっている)

    keep_dates = set(dates[-ROLLING_DAYS:])
    archive_rows = [r for r in rows if r["timestamp_jst"][:10] not in keep_dates]
    keep_rows = [r for r in rows if r["timestamp_jst"][:10] in keep_dates]

    # 月(YYYYMM)ごとにグループ化してアーカイブへ追記
    by_month = {}
    for r in archive_rows:
        ym = r["timestamp_jst"][:7].replace("-", "")  # "YYYY-MM" -> "YYYYMM"
        by_month.setdefault(ym, []).append(r)

    base = path[: -len(LIVE_SUFFIX)]  # data/jp_stocks/7203
    for ym, grp in by_month.items():
        archive_path = f"{base}{LIVE_SUFFIX[:-4]}_{ym}.csv.gz"  # {code}_T_1m_YYYYMM.csv.gz
